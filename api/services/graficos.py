...
# Configurar matplotlib para usar o backend "Agg" para renderizar gráficos em ambientes sem interface gráfica (sem exibição)
matplotlib.use("Agg") # Use a non-interactive backend for matplotlib (Quando usamos a biblioteca matplotlib em um ambiente sem interface gráfica, como servidores ou scripts em segundo plano, é necessário usar um backend não interativo. O backend "Agg" é uma escolha popular para gerar gráficos e salvá-los como arquivos de imagem, pois não requer uma janela de exibição.)
